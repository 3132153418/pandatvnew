...
//    JCMediaManager.mediaPlayer = null;
        ...
        L.d("JcFullScreenActivity", currentPosition + "");
        //得到反射对象之后设置一些成员变量
        pandaVedioPlayer.IF_CURRENT_IS_FULLSCREEN = true;//当前是否充满全屏
        pandaVedioPlayer.IF_FULLSCREEN_IS_DIRECTLY = DIRECT_FULLSCREEN;//是否直接充满全屏
...
//                pandaVedioPlayer.ivStart.performClick();
    ...
    public void addToCollect() {
        ...
        if (selector.size() == 0) {
            ...
            if (ym) {
                ym = false;
                Toast toast = Toast.makeText(JCFullScreenActivity.this, "恭喜您收藏成功啦", Toast.LENGTH_SHORT);
                toast.setGravity(Gravity.CENTER, 0, 0);
                View inflate = View.inflate(JCFullScreenActivity.this, com.jiyun.pandatv.jcvideoplayer_lib.R.layout.item_toast, null);
                ((TextView) inflate.findViewById(com.jiyun.pandatv.jcvideoplayer_lib.R.id.tv_toast)).setText("请勿重新收藏");
